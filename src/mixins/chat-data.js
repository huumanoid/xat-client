...
module.exports.bind = function chatData(user, opts) {
  ...
  user.on('data', function (data) {
    ...
    const Users = user.chatbox.users
    const Message = user.chatbox.messages

    let UserIndex

    if (data.i) {
      const b = data.i.attributes.b && data.i.attributes.b.split(';=')
    ...
    if (data.w) {
      const v = data.w.attributes.v.split(' ')
    ...
    if (data.m) {
      ...
      if (data.m.attributes.i != null) {
        maxIndex = data.m.attributes.i === '0' ? maxIndex + 1 : Math.max(data.m.attributes.i, maxIndex)
      }
      const e = data.m
      let u = e.attributes.u
      if (u) {
        u = u.split('_')[0]
      }
